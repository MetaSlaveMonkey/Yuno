...
class UserInteractionModule(commands.Cog, name="User Interactions"):
    ...
    async def cog_check(self, ctx: Context[Yuno]) -> bool:  # type: ignore
        if ctx.guild is None:
            raise commands.NoPrivateMessage(
                ":x: | This command is not available in DMs."
            )
        ...
        if not ctx.author in bot.cached_users:
            async with bot.pool.acquire() as conn:
                bot.cached_users[ctx.author.id] = await YUser.from_discord_user(
                    conn, ctx.author
                )
    ...
    async def get_user_language(self, user_id: int) -> str:
        ...
        async with self.bot.pool.acquire() as conn:
            record = await conn.fetchrow(
                "SELECT locale FROM users WHERE user_id = $1", user_id
            )
    ...
    async def _get_interaction_count(
        self, author: YUser, target: YUser, action: str
    ) -> int:
        if self.bot.pool is None:
        ...
        async with self.bot.pool.acquire() as conn:
            count = await self.interactions.get_action_count(
                conn, author.user_id, target.user_id, action
            )
    ...
    async def _get_interactions(self, author: YUser, target: YUser) -> List[str]:
        ...
        async with self.bot.pool.acquire() as conn:
            actions = await self.interactions.get_actions(
                conn, author.user_id, target.user_id
            )
    ...
    async def _get_embed(
        ...
        return await self.interactions.get_embed(
            pool, author, target, interaction_name, author_lang
        )

    async def insert_action(
        self, pool: asyncpg.Pool, author_id: int, target_id: int, action: str
    ) -> None:
        async with pool.acquire() as conn:
            await conn.execute(
                "SELECT insert_action_item($1, $2, $3)", author_id, target_id, action
            )
    ...
    async def hug(self, ctx: Context, target: discord.Member = FuzzyConverter) -> None:
        if self.bot.pool is None:
