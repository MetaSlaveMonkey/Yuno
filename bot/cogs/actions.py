...
class UserInteractionModule(commands.Cog, name="User Interactions"):
    ...
    async def ensure_relationship(self, interaction: Interaction) -> None:
        async with self.bot.pool.acquire() as conn:
            await conn.execute("SELECT ensure_relationship($1, $2, $3)", interaction.author.user_id, interaction.int_type, interaction.target.user_id)

    async def insert_action(self, interaction: Interaction) -> None:
        await self.ensure_relationship(interaction)

        async with self.bot.pool.acquire() as conn:
            await conn.execute("SELECT insert_action($1, $2, $3)", interaction.author.user_id, interaction.int_type, interaction.target.user_id)
